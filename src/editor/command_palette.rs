...
impl CommandPalette {
    ...
    pub fn selected_command_info(&mut self) -> &CommandInfo {
    ...
    pub fn update_filtered_commands(&mut self) {
        self.filtered_commands = self
            .commands
            .iter()
            .filter(|c| c.name.contains(&self.query))
            .cloned()
            .collect();
    ...
    /// Inserts a character into the query.
    pub fn insert_char(&mut self, c: char) {
        self.query.push(c);
        self.update_filtered_commands();
    }

    /// Deletes a character from the query.
    ...
    /// Selects the previous command in the command palette, wraoping around at the start.
